...
class Controller:
    ...
    def calculate_fitness(self):
        ...
        # V = average velocity of the two motors
        forwardFitness = np.absolute(np.average([self.velocity_left, self.velocity_right]))
                      
        ###########
        ### DEFINE the fitness function equation to avoid collision
        # normalise value of proximity sensors with higher activation
        values = []
        for sensor in self.proximity_sensors:
            values.append(sensor.getValue())
         
        normalised_proximity = [value / max(values) for value in values]
        avoidCollisionFitness = np.average(normalised_proximity) 
        
        ###########
        ### DEFINE the fitness function equation to avoid spining behaviour
        # absolute value of the speed difference between the two motors
        spinningFitness = np.absolute(self.velocity_left - self.velocity_right)
        
        ###########
        ### DEFINE the fitness function equation of this iteration which should be a combination of the previous functions
        combinedFitness = (forwardFitness * (1-np.sqrt(spinningFitness))) * (1-avoidCollisionFitness)
